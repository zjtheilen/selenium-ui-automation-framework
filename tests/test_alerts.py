...
def test_js_alert(driver, logger):
    ...
    logger.info("JS Alert test completed successfully with alert text: " + alert_text + " and result text: " + result_text)
...
def test_js_confirm_variations(driver, action, expected_text, logger):
    ...
    logger.info(f"JS Confirm test completed successfully with action: {action} and expected text: {expected_text}")
...
def test_js_prompt_variations(driver, input_text, expected, logger):
    ...
    logger.info(f"JS Prompt test completed successfully with input: '{input_text}' and expected result: '{expected}'")
